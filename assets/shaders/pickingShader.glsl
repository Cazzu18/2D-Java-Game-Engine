...
void main(){
    ...
    if(fTexId > 0){
        ...
        texColor = fColor * texture(uTextures[id], fTexCoords);//sampling color values from 2D texture to specific UV coordinates
        //color  = vec4(fTexCoords, 0, 1); //(x, y, 0, 1) rgba
    }

    // an alpha value(a) < 0.5 means that the texture is basically invisible
    //donut reference. Donut texture has a hole in the middle and that data is discarded while rest of donut data preserved
    if(texColor.a < 0.5){
        discard;
    }
